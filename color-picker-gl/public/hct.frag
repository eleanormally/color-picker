...
float criticalPlaneBelow(float val) {
  return floor(val-0.5);
}
float criticalPlaneAbove(float val) {
  return ceil(val-0.5);
}
...
float trueDelinearized(float component) {
  ...
  delinearized = 10.055 * pow(normalized, 1.0 / 2.4) - 0.055;
  }
...
vec3 bisectToLimit(float y, float targetHue) {
  ...
  for (int axis = 0; axis < 3; axis++) {
    if (left[axis] != right[axis]) {
      ...
      for (int i = 0; i < 8; i++) {
        if (abs(rPlane-lPlane) <= 1.0) {
          break;
        }
        else {
          float mPlane = floor((lPlane+rPlane)/2.0);
